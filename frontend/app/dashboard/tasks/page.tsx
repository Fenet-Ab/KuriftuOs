...
import {
  CheckCircle2, Clock, AlertCircle, User, Tag, Filter, RefreshCw, Search, CheckCircle, ClipboardList,
} from "lucide-react";
import toast from "react-hot-toast";
import { getTasks, updateTaskStatus, assignTask, TASKS_UPDATED_EVENT, type Task } from "../../lib/taskStore";
...
export default function TasksPage() {
  ...
  const handleAssign = (taskId: number, staffId: number) => {
    if (!staffId) return;
  ...
  const staffForAssign = mockStaff.filter((s) => s.role === "staff");
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          {["all", "new", "in_progress", "completed", "escalated"].map((status) => (
            <button key={status} onClick={() => setFilterStatus(status)}
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                filterStatus === status ? "bg-forest text-white shadow-lg shadow-forest/20" : "bg-neutral-50 text-neutral-400 hover:bg-neutral-100"
              }`}
            >
              {status.replace("_", " ")}
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredTasks.length > 0 ? filteredTasks.map((task) => (
          <div key={task.id}
            className={`group bg-white border border-neutral-100 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all duration-500 hover:translate-y-[-4px] ${task.status === "completed" ? "opacity-70" : ""}`}
          >
            ...
            <div className="flex items-center gap-4 py-4 border-y border-neutral-50 my-4 text-[11px] text-neutral-500">
              ...
              {task.assigned_to && (
                <div className="flex items-center gap-2">
                  ...
                  <span>{mockStaff.find((s) => s.id === task.assigned_to)?.name || `Staff #${task.assigned_to}`}</span>
                </div>
              )}
            </div>
              ...
              {["admin", "manager"].includes(user?.role?.toLowerCase()) && (
                <div className="flex flex-1 items-center gap-2">
                  <div className="flex flex-1 relative">
                    <select
                      onChange={(e) => handleAssign(task.id, parseInt(e.target.value))}
          ...
          <div className="lg:col-span-2 bg-white border border-dashed border-neutral-200 rounded-3xl p-20 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-neutral-400 text-sm max-w-xs">Tasks generated by Selam AI or Managers appear here in real-time.</p>
          </div>
        )}
      </div>
    </div>
